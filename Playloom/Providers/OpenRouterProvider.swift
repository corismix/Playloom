...
nonisolated final class OpenRouterProvider: ModelProvider, Sendable {
    ...
    Return only JSON matching {"title":String,"files":{"index.html":String,"game.js":String,"style.css":String}}. Build a touch-first Phaser 3 game. The complete game must be playable on an iPhone with visible on-screen controls or canvas pointer gestures; never make keyboard input the only control path. index.html loads vendor/phaser.min.js then game.js, has a local-only CSP, and a #game container. game.js exposes window.playloomProbeInput(), window.playloomRestart(), and window.playloomPixelSampleText(). Route real Phaser pointer/touch input and playloomProbeInput through the same gameplay handler, and have that handler send the input bridge event; do not emit input merely because the probe function was called. Send ready, heartbeat, input, restarted, fatal and console events through window.webkit.messageHandlers.playloom.postMessage. No remote URLs, modules, eval, storage, fetch, sockets, navigation, or external assets. Use procedural graphics.
    """
}
