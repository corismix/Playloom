...
@MainActor final class OpenCodeGoLiveSmokeTests:XCTestCase {
 func testOneRealGenerationPassesFullValidationLoop() async throws {
  guard let keyURL=Bundle.main.url(forResource:"live-smoke-key",withExtension:"txt"),
        let key=try? String(contentsOf:keyURL,encoding:.utf8).trimmingCharacters(in:.whitespacesAndNewlines),!key.isEmpty else { throw XCTSkip("No live key") }
